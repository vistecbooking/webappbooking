...
class ManageUsersPresenter extends ActionPresenter implements IManageUsersPresenter
{
    ...
    public function PageLoad()
    {
        if ($this->page->GetUserId() != null) {
            ...
            $userList = $this->userRepository->GetList($this->page->GetPageNumber(), $this->page->GetPageSize(),
                ...
                $this->page->GetSortDirection(), null, $this->page->GetFilterStatusId(), $this->page->GetFilterGroupId());
        }
    ...
    public function AddUser()
    {
        ...
        $user = $this->manageUsersService->AddUser(
            ...
            $this->page->GetNickName(),
            $this->page->GetLine(),
            $this->page->GetAdvisor(),
            $this->page->GetStudent(),
            $isAdmin);
    ...
    public function ImportUsers()
    {
        ...
        $positionsList = $this->positionViewRepository->GetList();
        $positions = $positionsList->Results();
        $positionsIndexed = array();
        foreach ($positions as $position) {
            $positionsIndexed[$position->Name()] = $position->Id();
        }
        ...
        for ($i = 0; $i < count($rows); $i++) 
        {
            ...
            try {
                ...
                $positionId = empty($row->position) ? $row->position : $positionsIndexed[$row->position];
                ...
                $user = $this->manageUsersService->AddUser($row->username, $row->email, $row->firstName, $row->lastName, $password, $timezone, $language,
                    ...
                    array(UserAttribute::Phone => $row->phone, UserAttribute::Organization => $row->organization, UserAttribute::PositionId => $positionId),
                    array());
