...
class ManageAnnouncementsPresenter extends ActionPresenter
{
    ...
    {
        $annouceList = $this->announcementRepository->GetList($this->page->GetPageNumber(), $this->page->GetPageSize(), $this->page->GetSortField(), $this->page->GetSortDirection());
        $this->page->BindAnnouncements($annouceList->Results());
        $this->page->BindPageInfo($annouceList->PageInfo());
    ...
    public function AddAnnouncement()
    {
        ...
        $timeStart = $this->page->GetTimeStart();
        $end = Date::Parse($this->page->GetEnd(), $user->Timezone);
        $timeend = $this->page->GetTimeEnd();
    ...
    public function ChangeAnnouncement()
    {
        ...
        $announcement->SetTimeStart($timeStart);
        $announcement->SetTimeEnd($timeend);
        $announcement->SetGroupIds($groupIds);
        $announcement->SetResourceIds($resourceIds);
    /*    $groupIds = $this->page->GetGroups();
        $resourceIds = $this->page->GetResources();*/
    ...
    {
        $id = $this->page->GetAnnouncementId();
        $CancelReason = $this->page->GetAnnouncementCancelReason();

        Log::Debug('Deleting Announcement with id %s', $id);

        $this->announcementRepository->Delete($id,$CancelReason);
    }
