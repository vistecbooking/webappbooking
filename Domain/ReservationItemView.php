...
class ReservationItemView implements IReservedItemView
{
	...
	public static function Populate($row)
	{
		...
		$view->Mode = $row[ColumnNames::MODE];
	 ...
	 * @return null|string
	 */
	public function GetColor($is_resource = false)
	{
		if($is_resource)
		{
			$group_id = 0;
			foreach($this->OwnerGroupIds() as $id)
			{
				$group_id = $id;
			}
	 ...
	 * @return string
	 */
	public function GetTextColor($is_resource = false)
	{
		$color = $this->GetColor($is_resource);
		if (!empty($color))
		{
