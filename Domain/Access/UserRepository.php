...
class UserFilter
{
    ...
    public function GetFilter()
    {
        ...
        if (!empty($this->position)) {
            $filter->_And(new SqlFilterEquals(ColumnNames::POSITION_ID, $this->position));
        }
...
class UserRepository implements IUserRepository, IAccountActivationRepository
{
    ...
    public function GetList($pageNumber, $pageSize, $sortField = null, $sortDirection = null, $filter = null,
                            $accountStatus = AccountStatus::ALL, $groupId = 0)
    {
        $command = new GetAllUsersByStatusCommand($accountStatus, $groupId);
    ...
    public function Add(User $user)
    {
        ...
        $id = $db->ExecuteInsert(new RegisterUserCommand($user->Username(), $user->EmailAddress(), $user->FirstName(),
            ...
            $user->GetAttribute(UserAttribute::PositionId), $user->StatusId(), $user->GetPublicId(),
            $user->GetDefaultScheduleId()));

        $user->WithId($id);

        if ($user->HaveUserPropChanged()) {
            $updateUserPropCommand = new UpdateUserPropCommand(
                $user->Id(),
                $user->NickName(),
                $user->LineID(),
                $user->AdvisorName(),
            ...
            $db->Execute($updateUserPropCommand);
        }
    ...
    public function Update(User $user)
    {
        ...
        if ($user->HaveAttributesChanged()) {
            $updateAttributesCommand = new UpdateUserAttributesCommand($userId, $user->GetAttribute(UserAttribute::Phone),
                $user->GetAttribute(UserAttribute::Organization),
                $user->GetAttribute(UserAttribute::PositionId));
    ...
    public function UpdateImage(User $user)
    {
        ...
        $db = ServiceLocator::GetDatabase();
        $updateUserImageCommand = new UpdateUserImageCommand($user->Id(), $user->ProfileImg());
        $db->Execute($updateUserImageCommand);
    }
...
class UserItemView
{
    ...
    public static function Create($row)
    {
        ...
        $user->IsAdmin = $row[ColumnNames::IS_ADMIN];
        ...
        $user->Position = $row[ColumnNames::POSITION_ID];
        $user->PositionName = $row[ColumnNames::POSITION_NAME_VIEW];
        $user->Language = $row[ColumnNames::LANGUAGE_CODE];
        $user->Advisor = $row[ColumnNames::ADVISOR_NAME];
        $user->LineID = $row[ColumnNames::LINE_ID];
        $user->StudentID = $row[ColumnNames::STUDENT_ID];
        $user->NickName = $row[ColumnNames::NICKNAME];
        $user->ProfileImg = $row[ColumnNames::PROFILE_IMG];
