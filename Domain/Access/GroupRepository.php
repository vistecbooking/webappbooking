...
class GroupRepository implements IGroupRepository, IGroupViewRepository
{
	...
	public function LoadById($groupId)
	{
		...
		{
			$group = new Group($row[ColumnNames::GROUP_ID], $row[ColumnNames::GROUP_NAME], $row[ColumnNames::GROUP_COLOR]);
	...
	public function Update(Group $group)
	{
		...
		$db->Execute(new UpdateGroupCommand($groupId, $group->Name(), $group->AdminGroupId(), $group->Color()));
	...
	public function Add(Group $group)
	{
		$groupId = ServiceLocator::GetDatabase()->ExecuteInsert(new AddGroupCommand($group->Name(), $group->Color()));
...
class GroupItemView
{
	public static function Create($row)
	{
		...
		return new GroupItemView($row[ColumnNames::GROUP_ID], $row[ColumnNames::GROUP_NAME], $adminName, $row[ColumnNames::GROUP_COLOR]);
	}
	...
	public function __construct($groupId, $groupName, $adminGroupName = null, $groupColor = "")
	{
		$this->Id = $groupId;
		$this->Name = $groupName;
