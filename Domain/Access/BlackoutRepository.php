...
interface IBlackoutRepository
{
     ...
     * @param int $blackoutId
     */
    public function DeleteSeriesAnnounce($blackoutId);
...
class BlackoutRepository implements IBlackoutRepository
{
	...
	private function AddSeries (BlackoutSeries $blackoutSeries)
	{
		...
		$seriesId = $db->ExecuteInsert(new AddBlackoutCommand($blackoutSeries->OwnerId(), $blackoutSeries->Title(), $blackoutSeries->RepeatType(), $blackoutSeries->RepeatConfigurationString(), $blackoutSeries->AnnounceId()));
	...
	public function DeleteSeriesAnnounce($blackoutId)
	{
		ServiceLocator::GetDatabase()->Execute(new DeleteBlackoutSeriesAndAnnounceCommand($blackoutId));
	}
