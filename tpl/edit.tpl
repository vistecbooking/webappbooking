...
{block name=header}
	{include file='globalheader.tpl' Validator=true TitleKey='EditReservationHeading' TitleArgs='' Qtip=true printCssFiles='css/reservation.print.css'}
{/block}
...
{block name=submitButtons}
	<div class="btn-group btnMore">
		{assign var=icsUrl value="{$Path}export/{Pages::CALENDAR_EXPORT}?{QueryStringKeys::REFERENCE_NUMBER}={$ReferenceNumber}"}
			<a  class="btn btn-default" href="{$icsUrl}" download="{$icsUrl}">
	...
	{if $IsRecurring}
		{if $ReservationAction == "update" && $CanEditDateTime}
			<button type="button" class="btn btn-danger triggerDeletePrompt delete prompt-single">
				...
				{translate key='ReservationCancel'}
			</button>
		{/if}
    ...
    <div id="deleteButtonPrompt" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <h3>{translate key=DeleteReminderWarning}</h3>
                 </div>
