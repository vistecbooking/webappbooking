...
<div class="form-group {if !$searchmode && $attribute->Required()}has-feedback{/if} {$class}">
	...
	{else}
		...
		<input type="hidden" id="formatted{$attributeId}" name="{$attributeName}" value="{formatdate date=$attributeValue key=system_datetime}" {if $attribute->Required() && !$searchmode}required="required" data-bv-notempty="true"{/if} />
