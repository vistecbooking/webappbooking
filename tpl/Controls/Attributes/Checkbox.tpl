...
<div class="form-group {$class}">
    ...
    {elseif $searchmode}
        ...
        <select id="{$attributeId}" name="{$attributeName}" class="customAttribute form-control {$inputClass}">
            <option value="">{translate key=SelectItem}</option>
    ...
    {else}
        <div class="form-check">
            <input type="checkbox" value="1" id="{$attributeId}" name="{$attributeName}" 
                   {if $attribute->Value() == "1"}checked="checked"{/if} class="form-check-input {$inputClass}"/>
            <label class="customAttribute form-check-label" for="{$attributeId}">
                {$attribute->Label()}
            </label>
        </div>
    {/if}
