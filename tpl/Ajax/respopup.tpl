...
{if $authorized}
    <div class="res_popup_details" style="margin:0">
        {if isset($resourceName)}
            <b><u>Instrument:</u></b> {$resourceName}<br/>
        {/if}
        <b><u>Name:</u></b> {$fullName}<br/>
        <b><u>Research group:</u></b> {$resourcesGroupName}</br>
        <b><u>Phone no.:</u></b> {$phone}</br>
        {if !empty($mode)}
            <b><u>Mode:</u></b> {$mode}</br>
        {/if}
        <b><u>Total hours (of your group):</u></b> {$sum_hour}</br>
                {assign var="key" value="res_popup"}
                {if $startDate->DateEquals($endDate)}
                    {assign var="key" value="res_popup_time"}
                {/if}
        <b><u>Selected hours (show as duration):</u></b> </br>{formatdate date=$startDate key=res_popup} - {formatdate date=$endDate key=$key} ({$duration})</br>
        <b><u>Remaining hours:</u></b> {$quota_limit} </br>
        {capture "name"}
            <div class="user">
        ...
        {capture "title"}
            {if !$hideDetails}
                <div class="title">{if $title neq ''}{$title}{else}{translate key=NoTitleLabel}asdasdas{/if}</div>
            {/if}
        {/capture}
