				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="row">
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									<label class="reg" for="addNickname">{translate key="NickName"}</label>
									<input type="text" {formname key="NICKNAME"} class="form-control" id="addNickname"/>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									<label class="reg" for="addPhone">{translate key="Phone"}</label>
									<input type="text" {formname key="PHONE"} class="form-control" id="addPhone"/>
								</div>
							</div>
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									<label class="reg" for="addLineID">{translate key="LineID"}</label>
									<input type="text" {formname key="LINE_ID"} class="form-control" id="addLineID"/>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									<label class="reg" for="addAdvisorName">{translate key="AdvisorName"}</label>
									<input type="text" {formname key="ADVISOR_NAME"} class="form-control" id="addAdvisorName"/>
								</div>
							</div>
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									<label class="reg" for="addStudentID">{translate key="StudentID"}</label>
									<input type="text" {formname key="STUDENT_ID"} class="form-control" id="addStudentID"/>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									<label class="reg" for="addOrganization">{translate key="Organization"}</label>
									<input type="text" {formname key="ORGANIZATION"} class="form-control" id="addOrganization"/>
								</div>
							</div>
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									<label class="reg" for="addPosition">{translate key="Position"}</label>
									<select id="addPosition" {formname key='POSITION'} class="form-control">
						...
						{if $AttributeList|count > 0}
							{for $i=1 to $AttributeList|count}
								...
								{/if}
								<div class="col-sm-12 col-md-6">
									{control type="AttributeControl" attribute=$AttributeList[$i-1]}
								</div>
								{if $i%2==0 || $i==$AttributeList|count}
									</div>
								{/if}
							{/for}
						{/if}
					...
					<div class="modal-footer">
						<div class="checkbox inline">
                            <input type="checkbox" id="addIsAdmin" {formname key=IS_ADMIN} />
                            <label for="addIsAdmin">{translate key=IsAdmin}</label>
                        </div>
						&nbsp;&nbsp;&nbsp;&nbsp;
                        <div class="checkbox inline">
		...
		<form id="userForm" method="post" ajaxAction="{ManageUsersActions::UpdateUser}">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
						...
						<div class="row">
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									...
									<input type="text" {formname key="NICKNAME"} class="form-control" id="nickname"/>
								</div>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									...
									<input type="text" {formname key="LINE_ID"} class="form-control" id="line_ID"/>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									...
									<input type="text" {formname key="ADVISOR_NAME"} class="form-control" id="advisor_name"/>
								</div>
							</div>
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									<label class="reg" for="student_ID">{translate key="StudentID"}</label>
									<input type="text" {formname key="STUDENT_ID"} class="form-control" id="student_ID"/>
								</div>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-xs-12 col-sm-6">
								<div class="form-group">
									...
									<select id="position" {formname key='POSITION'} class="form-control">
										<option value="0">{translate key=None}</option>
										{object_html_options options=$Positions label=Name key=Id}
									</select>
								</div>
							</div>
						</div>
					...
					<div class="modal-footer">
						<div class="checkbox inline">
                            <input type="checkbox" id="is_admin" {formname key=IS_ADMIN} />
                            <label for="is_admin">{translate key=IsAdmin}</label>
                        </div>
						&nbsp;&nbsp;&nbsp;&nbsp;
						{cancel_button}
						{update_button}
	...
	<div id="imgDialog" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="imgModalLabel"
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="modal-title" id="imgModalLabel">{translate key=ProfileImg}</h4>
				</div>
				<div class="modal-body">
					<center>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			var userOptions = {
				...
				filterUrl: '{$smarty.server.SCRIPT_NAME}?',
				queryStatus: '{QueryStringKeys::ACCOUNT_STATUS}=',
				queryGroup: '{QueryStringKeys::GROUP_ID}=',
				actions: actions,
			...
			var user = {
				...
				nickname: '{$user->NickName|escape:"quotes"}',
				lineid: '{$user->LineID|escape:"quotes"}',
				studentid: '{$user->StudentID|escape:"quotes"}',
				advisor: '{$user->Advisor|escape:"quotes"}',
				reservationColor: '{$user->ReservationColor|escape:"quotes"}',
				profileImg: '{profile_image image=$user->ProfileImg}'
			};
