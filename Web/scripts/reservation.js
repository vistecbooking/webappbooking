function Reservation(opts) {
	...
	var defaultSubmitCallback = function (form) {
		...
		if(target == "reservation_delete.php")
		{
	...
	{
		if(data.ErrorIds.length > 0 && data.Messages[0] == undefined)
		{
			hideModal();
	...
	function hideModal()
	{
		...
		var top = $("#reservation-box").scrollTop();
	...
	function onBeforeSubmit(formData, jqForm, opts)
	{
		...
		if(res.length == 2)
		{
			...
			{
				if (bv.$invalidFields.length > 0 || !bv.isValid())
				{
					return false;
				}
			}
