function UserManagement(opts) {
	...
	UserManagement.prototype.init = function () {
		...
		elements.userList.delegate('.viewimg', 'click', function (e) {
			var user = getActiveUser();
			$('#profile-img').attr("src", user.profileImg);
			elements.imgDialog.modal('show');
		});
		...
		elements.filterStatusId.change(function () {
			...
			var groupid = elements.filterGroupId.val();
			var status = options.queryStatus + statusid;
			var group = options.queryGroup + groupid;
			window.location.href = options.filterUrl + status + "&" + group;
		});

		elements.filterGroupId.change(function () {
			var statusid = elements.filterStatusId.val();
	...
	var changeUserInfo = function () {
		...
		$('#position').val(user.position != "" ? user.position : "0");
		$('#nickname').val(user.nickname);
		$('#line_ID').val(user.lineid);
		$('#advisor_name').val(user.advisor);
		$('#student_ID').val(user.studentid);
