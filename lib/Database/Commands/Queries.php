...
class Queries
{
	...
	const ADD_ANNOUNCEMENT =
			'INSERT INTO announcements (announcement_text, priority, start_date, end_date, start_time,end_time )
		VALUES (@text, @priority, @startDate, @endDate,@timeStart,@timeEnd)';
	...
	const ADD_BLACKOUT_SERIES =
			'INSERT INTO blackout_series (date_created, title, owner_id, repeat_type, repeat_options, announce_id) VALUES (@dateCreated, @title, @userid, @repeatType, @repeatOptions, @announcementid)';

	const ADD_GROUP =
			'INSERT INTO groups (name, color) VALUES (@groupname, @groupcolor)';
	...
	const ADD_RESERVATION_SERIES =
		...
		VALUES (@dateCreated, @title, @description, @allow_participation, false, @repeatType, @repeatOptions, @typeid, @statusid, @userid, @mode)';
	...
	const ADD_POSITION =
			'INSERT INTO positions (name) VALUES (@positionname)';
	...
	const DELETE_ANNOUNCEMENT = 'UPDATE announcements SET status_deleted = "1",cancel_reason = @cancelReason WHERE announcementid = @announcementid';
	...
	const DELETE_BLACKOUT_ANNOUNCE_SERIES = 'DELETE blackout_series, announcements FROM blackout_series
		INNER JOIN blackout_instances ON blackout_series.blackout_series_id = blackout_instances.blackout_series_id
	...
	const GET_ALL_ANNOUNCEMENTS = 'SELECT a.*, 
			...
			FROM announcements a WHERE a.status_deleted != "1"  OR a.status_deleted IS NULL ORDER BY start_date DESC';

	const GET_ALL_ADMIN = 'SELECT *
            FROM users
            WHERE status_id = @user_statusid AND is_admin = true';
	...
	const GET_ALL_USERS_BY_STATUS =
			'SELECT u.*, pt.name AS position_name,
	...
	const GET_ALL_USERS_BY_STATUS_AND_GROUP =
			'SELECT u.*, pt.name AS position_name,
			(SELECT GROUP_CONCAT(CONCAT(p.name, "=", p.value) SEPARATOR "!sep!")
						FROM user_preferences p WHERE u.user_id = p.user_id) as preferences,
			(SELECT GROUP_CONCAT(CONCAT(cav.custom_attribute_id, \'=\', cav.attribute_value) SEPARATOR "!sep!")
			...
			WHERE (0 = @user_statusid OR status_id = @user_statusid) AND ugs.group_id = @groupid ORDER BY lname, fname';

const GET_ALL_USERS_BY_STATUS_AND_NO_GROUP =
			'SELECT u.*, pt.name AS position_name,
			(SELECT GROUP_CONCAT(CONCAT(p.name, "=", p.value) SEPARATOR "!sep!")
						FROM user_preferences p WHERE u.user_id = p.user_id) as preferences,
			(SELECT GROUP_CONCAT(CONCAT(cav.custom_attribute_id, \'=\', cav.attribute_value) SEPARATOR "!sep!")
			...
			LEFT JOIN positions pt ON pt.position_id = u.position_id 
			LEFT JOIN user_groups ugs ON ugs.user_id = u.user_id 
			WHERE (0 = @user_statusid OR status_id = @user_statusid) AND ugs.group_id IS NULL ORDER BY lname, fname';

	const GET_ANNOUNCEMENT_BY_ID = 'SELECT a.*,
 		(SELECT GROUP_CONCAT(ag.group_id) FROM announcement_groups ag WHERE ag.announcementid = a.announcementid) as group_ids,
		(SELECT GROUP_CONCAT(ar.resource_id) FROM announcement_resources ar WHERE ar.announcementid = a.announcementid) as resource_ids
		FROM announcements a WHERE a.announcementid = @announcementid AND (a.status_deleted != "1" OR a.status_deleted IS NULL)';
	...
	const GET_DASHBOARD_ANNOUNCEMENTS =
		...
		WHERE (CONVERT(CONCAT(DATE_FORMAT(start_date, "%Y-%m-%d"), " " ,start_time), DATETIME) <= @current_date AND CONVERT(CONCAT(DATE_FORMAT(end_date, "%Y-%m-%d"), " " ,end_time), DATETIME) >= @current_date AND (a.status_deleted != "1" OR a.status_deleted IS NULL)) OR (end_date IS NULL)
	...
	const GET_RESERVATION_FOR_EDITING =
			'SELECT ri.*, rs.*, rr.*, u.user_id, u.fname, u.lname, u.email, u.phone, r.schedule_id, r.name, rs.status_id as status_id,g.group_id,g.name AS name_group
		FROM reservation_instances ri
...
//			ORDER BY ri.start_date ASC';
	...
	const REGISTER_USER =
		...
		VALUES
			(@email, @password, @fname, @lname, @phone, @organization, @positionid, @username, @salt, @timezone, @language, @homepageid, @user_statusid, @dateCreated, @publicid, @scheduleid)';
	...
	const UPDATE_GROUP =
			'UPDATE groups
		SET name = @groupname, admin_group_id = @admin_group_id, color = @groupcolor 
		WHERE group_id = @groupid';
