...
class CalendarReservation
{
	 ...
	 * @return CalendarReservation[]
	 */
	public static function FromScheduleReservationList($reservations, $resources, UserSession $userSession, $groupSeriesByResource = false, $filterResource = '')
	{
		$knownSeries = array();
		...
		foreach ($reservations as $reservation)
		{
			...
			$color = $reservation->GetColor($filterResource != '');
			if (!empty($color))
			{
				$cr->Color = $reservation->GetColor($filterResource != '') . ' !important';
				$cr->TextColor = $reservation->GetTextColor($filterResource != '') . ' !important';
			}

			$cr->IsEditable = $userSession->IsAdmin || $userSession->UserId == $reservation->OwnerId;

			$cr->Class = self::GetClass($reservation);
			$cr->IsCalendarResource = true;
	...
	/**
	 * @param $openings AvailableOpeningView
	 * @param $timezone string
	 * @param $user UserSession
	 * @return CalendarReservation
	 */
	public static function FromViewFindATimeList($openings, $timezone, $user)
	{
		$results = array();
		$factory = new SlotLabelFactory($user);
		...
		for ($i = 0; $i < count($openings); $i++)
		{
			...
			$resourceName = $openresource->GetName();
			$referenceNumber = null;

			$res = new CalendarReservation($start, $end, $resourceName, $referenceNumber);
			$res->ResourceId = $openresource->GetResourceId();
			$res->Title = $resourceName;
			$res->Description = null;
			$res->DisplayTitle = $resourceName;
			$res->Invited = null;
			$res->Participant = null;
			$res->Owner = null;
			$res->Color = $openresource->GetColor() . ' !important';
			$res->TextColor = $openresource->GetTextColor() . ' !important';
			$res->IsEditable = $user->IsAdmin;
			$res->Class = 'reserved';
			$res->IsCalendarResource = false;
	...
	public function AsFullCalendarEvent()
	{
		...
		return array(
				...
				'isCalendarResource' => $this->IsCalendarResource
		);
	}

	public function AsFullCalendarFindATimeEvent()
	{
		$dateFormat = Resources::GetInstance()->GetDateFormat('fullcalendar');
		return array(
				'id' => $this->ResourceId,
				...
				'start' => $this->StartDate->Format($dateFormat),
				'end' => $this->EndDate->Format($dateFormat),
				'url' => sprintf('%s?rid=%s&sd=%s&ed=%s', Pages::RESERVATION, $this->ResourceId, $this->StartDate->Format($dateFormat), $this->EndDate->Format($dateFormat)),
				'allDay' => false,
				'backgroundColor' => $this->Color,
				'textColor' => $this->TextColor,
				'className' => $this->Class,
