...
class ReservationDateBinder implements IReservationComponentBinder
{
	...
	public function Bind(IReservationComponentInitializer $initializer)
	{
		...
		if($initializer->GetResourceId() == 2 || $initializer->GetResourceId() == 24){
			$hideRecurrence = true;
		}
		else{
...
class ReservationDetailsBinder implements IReservationComponentBinder
{
	...
	public function Bind(IReservationComponentInitializer $initializer)
	{
		...
		if($this->reservationView->Mode != null && $this->reservationView->Mode != ""){
			$this->page->SetMode($this->reservationView->Mode);
		}
		...
		$day1 = strtotime(Date::Now());
		$day2 = strtotime($this->reservationView->StartDate);
		$minustime = $day2 - $day1;
		if($minustime < 0){
			$minustime = $minustime * -1;
		}
		$diffHours = round($minustime / 3600);
		$canEditDateTime = false;
		if($currentUser->IsAdmin || (!$currentUser->IsAdmin && $diffHours >= 24)){
			$canEditDateTime = true;
		}

		$this->page->SetCanEditDateTime($canEditDateTime);
	}
